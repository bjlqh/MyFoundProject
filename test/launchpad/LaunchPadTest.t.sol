...
contract MockRouter {
    ...
    function swapExactETHForTokens(
        ...
        if (path.length > 1) {
            ...
            // 由于LaunchPadToken的mintForLiquidity函数只能由launchPad调用，我们需要用其他方式模拟转移
...
contract LaunchPadTest is Test {
    ...
    function testLiquidityAddition() public {
        ...
        for (uint i = 0; i < 10; i++) {
            ...
            vm.prank(testBuyer);
            launchPad.buyToken{value: 0.1 ether}(token);
        }

        //验证流动性已添加
        assertTrue(launchPad.liquidityAdded(token));
    ...
    function testBuyMeme() public {
        ...
        vm.prank(address(launchPad)); // 只有launchPad可以mint
        ...
        //执行buyMeme操作
        vm.prank(buyer);
        launchPad.buyMeme{value: 0.05 ether}(token);
    ...
    function testGetUniswapPrice() public {
        ...
        //查看uniswap价格
        uint uniswapPrice = launchPad.getUniswapPrice(token);
        console.log("Uniswap price:", uniswapPrice);
        assertTrue(uniswapPrice > 0);

        //检查流动性状态
        assertTrue(launchPad.hashUniswapLiquidity(token));
    }
}
